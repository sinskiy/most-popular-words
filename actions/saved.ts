...
export const getSaved = cacheDb(
  async (offset: number, username: string, limit?: number) =>
    await queryThrowError<SavedWord>(
      ...
      "SELECT value, occurrences, percentage, source, type, knowledge, translations, definitions, examples, saved FROM user_words_with_percentage($1) WHERE saved = true LIMIT $2 OFFSET $3",
...
export const getSavedWithLanguage = cacheDb(
  async (offset: number, username: string, language: string, limit?: number) =>
    await queryThrowError<SavedWord>(
      "Couldn't get saved words",
      "SELECT value, occurrences, percentage, source, type, knowledge, translations, definitions, examples, saved FROM user_words_with_percentage($2) WHERE LANGUAGE = $1 AND saved = true LIMIT $3 OFFSET $4",
